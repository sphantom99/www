...
export async function getAdminStatsDiagram(typeParam, dayParam, methodParam, ispParam) {
  ...
  const typeQuery = typeParam && { 'response.headers.contentType': typeParam };
  const dayQuery = dayParam && { day: dayParam };
  const methodQuery = methodParam && { 'request.method': methodParam };
  const ispQuery = ispParam && { isp: ispParam };
