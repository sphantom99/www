...
export async function updateUser(password, user, newUsername) {
  const url = 'mongodb://localhost:27017';
  const client = await MongoClient.connect(url, { useNewUrlParser: true }).catch((err) => {
    console.log(err);
  });
  ...
  try {
    ...
    const result = await collection.updateOne(
      { username: user },
      {
        $set: {
          encryptedPassword: password,
          username: newUsername !== user ? newUsername : user,
        },
      },
    );
    const entries = db.collection('entries');
    if (user !== newUsername) {
      const result2 = await entries.updateMany({ username: user }, { $set: { username: newUsername } });
    }
    // console.log(result);
    return result;
  } catch (err) {
...
export async function getIpsByUser() {
  ...
  try {
    ...
    const ipsPerClientCoordinatesWithCount = ipsPerClientCoordinates.map((item) => ({
      client: item.client,
    ...
    for (i = 0; i < rounds; i++) {
      if (i !== rounds - 1) {
        await axios
          .post('http://ip-api.com/batch', validIps.slice(i * 100, (i + 1) * 100), {
            headers: {
          ...
          .then((response) => {
            if (response.status === 200) {
              ipapiCoordinateResponse = ipapiCoordinateResponse.concat(response.data);
...
export async function getUniqueIps(user) {
  ...
  try {
    ...
    const ipList = ipapiCoordinateResponse.map((item) => ({ lat: item.lat, long: item.lon }));
    return ipList;
  } catch (err) {
    console.log(err);
    return [];
