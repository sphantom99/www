...
export default async function cleanFile(file) {
  ...
  const checkedMyIsp = await fetch(`http://ip-api.com/json/${ip}`, {
    method: 'GET',
    credentials: 'omit',
  });
  const { isp } = await checkedMyIsp.json();

  const cleanHarAfterFormat = log.entries.map((iter) => ({
    username: cook[0],
    clientIp: ip,
    clientCoordinates: {
      lat: latitude,
  ...
  console.log(cleanHarAfterFormat);
  const data = JSON.stringify(cleanHarAfterFormat);
